...
| Structured storage | Account-bound message envelopes and cached bodies | Per-account SQLCipher; exact schema and integrity validation; persistent encrypted WAL; in-memory temp policy; metadata-only read capability separate from body/mutation authority | macOS account store and strict reader implemented; Keychain-derived production keys, fixed App Group profile resolution, global store, drafts, pending operations, File Protection evidence, and device runtime remain planned |
