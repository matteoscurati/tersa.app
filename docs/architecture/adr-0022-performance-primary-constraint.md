...
Phase 1. Today they are governed only as a deferred final acceptance claim: the
[macOS acceptance protocol](../quality/macos-acceptance.md)
measures the release-equivalent signed candidate at the end, the
[product constraints](adr-0006-product-constraints.md) (ADR-0006 A5) hold the
cache budgets as constraints, and the
...
without moving any acceptance claim or changing any threshold.
...
which carries dual iPhone and Mac columns, with collection details in the
[macOS performance harness](../quality/macos-performance.md). This ADR
references those documents by name and restates no threshold value; per the
precedent of the
[macOS production UI toolkit ADR](adr-0020-macos-production-ui-toolkit.md),
the performance budgets are the acceptance bar by reference and are not
restated here. The cache budgets and their "constraints, not a pass" status
are ADR-0006 A5; this ADR neither restates nor bypasses them.
...
a peer in force to ADR-0006 A5 and A9, defined by reference to the
[macOS acceptance protocol](../quality/macos-acceptance.md) and the
canonical table it derives from. This ADR restates no threshold and changes
none: the acceptance protocol states that a threshold miss fails acceptance
unless a separately accepted ADR changes that budget, and this ADR is not that
change.
...
reviewer-enforced, recorded in the pull request, and never treated as
distribution-signed acceptance evidence.
...
protocol numbers act as a merge-time tripwire, not an acceptance result, and a
...
Development-signed measurements can never count toward macOS UI acceptance,
...
Cache measurement remains a product constraint and is not bypassed.
The macOS public MVP completion scope is unchanged: this ADR adds an
early development regime; it does not move any acceptance claim or rephase the
roadmap.

## Non-claims

This ADR passes, reopens, closes, downgrades, or edits no acceptance claim;
the mobile-inclusive production UI baseline stays unapproved, M1 stays
blocked, and the Phase 1 aggregate stays open.
...
This ADR makes no mobile or Phase 2 change; the "App Store last"
...
Developer ID direct download. This DMG note asserts no new acceptance evidence:
macOS release acceptance still validates and staples `Tersa.app`, and any DMG
