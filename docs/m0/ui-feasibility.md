...
`sh apple/scripts/capture-slint-evidence.sh`. That local-only path treats
screenshots as evidence only when Vision OCR finds the product marker and
visible diagnostic rows. The exact 10,000-row label is also required in each
...
the local capture. It is not merge-blocking CI and does not upload artifacts.
